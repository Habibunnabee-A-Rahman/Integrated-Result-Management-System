...
public class UpdateStudentFrame extends javax.swing.JFrame {
    ...
    public UpdateStudentFrame(String idT01,String idT06,String token) {
        initComponents();
        this.idT01 = idT01;
        this.idT06 = idT06;
        this.token = token;
        ...
        try {
            ...
            ps = connect.prepareStatement("SELECT t17_batch.T17_id, t17_batch.batch_id, t17_batch.batch_name  "
                    + "FROM `t17_batch` JOIN t14_semester ON t17_batch.T14_id_fk = t14_semester.T14_id "
            ...
            if (count > 0) {
                ...
                JOptionPane.showMessageDialog(this, "NO Batch Exists", "Error!", JOptionPane.ERROR_MESSAGE, error_icon);
                throw new SQLException("Batch Problem!");
            }
            ...
            if (select == 0) {
                UpdateStudentFrame uempfrm = new UpdateStudentFrame(this.idT01,this.idT06,this.token);
    ...
    private void initComponents() {
        ...
        setTitle("Student");
        setPreferredSize(new java.awt.Dimension(542, 594));
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icon/student_64.png"))); // NOI18N
        ...
        batchComboBox.setPreferredSize(new java.awt.Dimension(150, 30));
        ...
        mobileTextField.setFont(new java.awt.Font("SansSerif", 0, 12)); // NOI18N
        mobileTextField.setPreferredSize(new java.awt.Dimension(150, 30));
        jPanel7.add(mobileTextField);
        ...
        addressTextField.setFont(new java.awt.Font("SansSerif", 0, 12)); // NOI18N
        addressTextField.setPreferredSize(new java.awt.Dimension(300, 30));
        jPanel8.add(addressTextField);
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        ...
        if (batchComboBox.getSelectedItem().toString().trim().equals("None!")) {
            ImageIcon error = new ImageIcon(getClass().getResource("/icon/error_2_64.png"));
            JOptionPane.showMessageDialog(this, "Batch Doesn't Exist", "Error!", JOptionPane.ERROR_MESSAGE, error);
            return;
        }
        ...
        String mobileNo = mobileTextField.getText().trim();
        String address = addressTextField.getText().trim();
        int batch_index = batchComboBox.getSelectedIndex();
        
        if(stuID.isBlank()||name.isBlank()||mobileNo.isBlank()){
            ImageIcon error = new ImageIcon(getClass().getResource("/icon/error_2_64.png"));
        ...
        try {
            ...
            if(!idT20.equals("None!") && !idT20.trim().isBlank()){
                ps = connect.prepareStatement("UPDATE `t20_student` "
                        + "SET `T17_id_fk`=?,`student_name`=?,`mobile`=?,`address`=? "
                        + "WHERE T20_id = ? AND (student_status IS NULL OR student_status < 1)");
                ps.setString(1, T17_id[batch_index]);
                ps.setString(2, name);
                ps.setString(3, mobileNo);
                ps.setString(4, address);
                ps.setString(5, idT20);
                if(ps.executeUpdate()<=0){
                    throw new SQLException("Update Error!");
                }
    
            }else{
                ps = connect.prepareStatement("INSERT INTO `t20_student`"
                        + "(`student_id`, `T01_id_fk`, `T17_id_fk`, `student_name`, `mobile`, `address`) "
                        + "VALUES (?,?,?,?,?,?)");
                ps.setString(1, stuID);
                ps.setString(2,idT01);
                ps.setString(3, T17_id[batch_index]);
                ps.setString(4, name);
            ...
            if(login){
                if(loginCheckBox.isSelected()){
                    if(setPasswordCheckBox.isSelected()){
                        ps = connect.prepareStatement("UPDATE `student_login` SET `password`= ? "
                    ...
                    ps = connect.prepareStatement("DELETE FROM `student_login` "
                            + "WHERE ID = ? AND university_id = ?");
                    ps.setString(1, stuID);
                ...
                if(loginCheckBox.isSelected() && setPasswordCheckBox.isSelected()){
                    String temp_idT20s = "";
                    ps = connect.prepareStatement("SELECT T20_id FROM `t20_student` "
                            + "WHERE student_id = ? AND T01_id_fk = ?");
                    ps.setString(1, stuID);
                    ps.setString(2, idT01);
                    ResultSet rs = ps.executeQuery();
                    while(rs.next()){
                        temp_idT20s = rs.getString(1);
                    }
                    
                    ps = connect.prepareStatement("INSERT INTO `student_login`(`ID`, `university_id`, `password`,`T20_id_fk`)"
                            + " VALUES (?,?,?,?)");
                    ps.setString(1, stuID);
                    ps.setString(2, uni_id);
                    ps.setString(4, temp_idT20s);
            ...
            JOptionPane.showMessageDialog(this, "Successful Student Update!", "Success!", JOptionPane.ERROR_MESSAGE, success);
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        // TODO add your handling code here:
        ImageIcon error = new ImageIcon(getClass().getResource("/icon/error_2_64.png"));          
        if(idT20.equals("None!") || idT20.isBlank() ){
        ...
        try {
            ...
            //System.out.println(idT20);
            ps = connect.prepareStatement("DELETE FROM `t20_student` WHERE T20_id = ? AND student_status <1");
            ps.setString(1, idT20);
            ...
            ImageIcon success = new ImageIcon(getClass().getResource("/icon/success_64.png"));
            JOptionPane.showMessageDialog(this, "Deleting Student Successful!", "Success!", JOptionPane.ERROR_MESSAGE, success);
    ...
    private void idTextFieldKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_idTextFieldKeyReleased
        ...
        String temp_stuid = idTextField.getText().trim();
        idT20 = "None!";
        stu_status =1;
        try{
            ps = connect.prepareStatement("SELECT T20_id,T17_id_fk,student_name,mobile,address,student_status FROM `t20_student` "
                    + "WHERE student_id = ? AND T01_id_fk = ?");
            ps.setString(1, temp_stuid);
            ps.setString(2, idT01);
            ResultSet rs = ps.executeQuery();
            if(rs.next()){
                idT20 = rs.getString(1);
                for(int i=0;i<T17_id.length;i++){
                    if(T17_id[i].equals(rs.getString(2))){
                        batchComboBox.setSelectedIndex(i);
                        break;
                    }else if(i==T17_id.length-1){
                        saveButton.setEnabled(false);
                        deleteButton.setEnabled(false);
                        ImageIcon error = new ImageIcon(getClass().getResource("/icon/error_2_64.png"));
                        JOptionPane.showMessageDialog(this, "This ID exists in another Syllabus", "Error!", JOptionPane.ERROR_MESSAGE, error);
                        return;
                    }
                }
                nameTextField.setText(rs.getString(3));
                mobileTextField.setText(rs.getString(4));
                addressTextField.setText(rs.getString(5));
                stu_status = rs.getInt(6);
                if(rs.getInt(6)>0){
                    deleteButton.setEnabled(false);
                }else{
                ...
                //check Login Table
                ps = connect.prepareStatement("SELECT * FROM `student_login` "
                        + "WHERE ID = ? AND university_id = ?");
                ps.setString(1, temp_stuid);
                ps.setString(2, uni_id);
                rs = ps.executeQuery();
                ...
                idT20 = "None!";
                nameTextField.setText("");
                batchComboBox.setSelectedIndex(batchComboBox.getSelectedIndex());
                mobileTextField.setText("");
                addressTextField.setText("");
            }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new UpdateStudentFrame("default","default","default").setVisible(true);
            }
        });
    }
